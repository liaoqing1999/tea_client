...
import FProcess from './fProcess';
import UFProcess from './ufProcess';
import { reqOrg, reqDictType } from '../../../../api';
import AddUFProcess from './addUFProcess';
const { TabPane } = Tabs;
export default class Process extends Component {
  ...
  componentDidMount = () => {
    let typeCodes = ["type", "process","grade"]
  ...
  render() {
    ...
    return (
      <div>
        ...
        >
          <AddUFProcess dict={dict} addRefresh={this.addRefresh}  hideModal={() => this.setState({ addVisible: false })}></AddUFProcess>
        </Modal>
        {user.org ? (
          <Tabs defaultActiveKey="1" tabBarExtraContent={operations}>
            <TabPane tab="待办" key="1">
              <UFProcess addRefresh={this.addRefresh} refresh={refresh} dict={dict}></UFProcess>
            </TabPane>
            <TabPane tab="已完成" key="2">
              <FProcess refresh={refresh} dict={dict}></FProcess>
            </TabPane>
          </Tabs>
        ) :
          (<Alert
